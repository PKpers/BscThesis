...
inFile = inPath + "WPhiJets_M60M5080Deltas_Application_Smeared0HistFit.root" 

outName = "WPhiJets_M60M5080_SampleFitWArrows.pdf"
size = 0.045
c = ROOT.TCanvas()
...
arrow_y_start = y_min + 0.2 * (y_max - y_min)
arrow_y_end = y_min + 0.3 * (y_max - y_min)

fbArrow = ROOT.TArrow(arrow_x_start, arrow_y_start, arrow_x_end, arrow_y_end, 0.02,"|>")
fbArrow.SetLineWidth(2)
fbArrow.SetFillColor(4)
fbArrow.DrawClone()

# plot the corresponding text
fbText = ROOT.TLatex()
fbText.SetTextSize(0.035)
fbText.DrawLatexNDC(0.4, 0.2, "#color[4]{ #splitline{Modeled}{Background} } " )
c.Update()
c.SaveAs(outName)
